...
function getModeQuery() {
    return getMode().toLowerCase().includes('take') ? 'takeout' : 'dine-in';
}
...
function editOrder() {
    window.location.href = `menu.html?mode=${getModeQuery()}`;
}
...
function wireButtons() {
    const checkoutLink = document.querySelector('.cart-buttons a[href="payment.html"]');
    if (checkoutLink) {
...
function updateButtonLinks() {
    ...
    if (backLink) {
        backLink.href = `menu.html?mode=${getModeQuery()}`;
    }
}
